...
package com.sourcery.magiccontrol.util;
...
public class Helpersled {
    ...
    */
    ...
    public static boolean checkBusybox() {
        ...
        try {
            if (!new DENSITYCMDProcessor().su.runWaitFor("busybox mount").success()) {
    ...
    {
        final DENSITYCMDProcessor cmd = new DENSITYCMDProcessor();
    ...
    public static String readFileViaShell(String filePath, boolean useSu) {
        DENSITYCMDProcessor.CommandResult cr = null;
        if (useSu) {
            cr = new DENSITYCMDProcessor().su.runWaitFor("cat " + filePath);
        } else {
            cr = new DENSITYCMDProcessor().sh.runWaitFor("cat " + filePath);
        }
        if (cr.success())
            return cr.stdout;
        return null;
    ...
    public static void restartSystemUI() {
        new DENSITYCMDProcessor().su.run("pkill -TERM -f com.android.systemui");
    }

    public static void setSystemProp(String prop, String val) {
        new DENSITYCMDProcessor().su.run("setprop " + prop + " " + val);
    }
    ...
    private static String getSystemProp(String prop) {
        CommandResult cr = new DENSITYCMDProcessor().sh.runWaitFor("getprop " + prop);
        if (cr.success()) {
            return cr.stdout;
        } else {
            return null;
